...
function setThemeMode(mode) {
    const B = document.documentElement; // Root element (<html>)
    if (mode === "dark") {
        B.setAttribute("data-bs-theme", "dark");
        localStorage.setItem("theme", "dark"); // Simpan mode ke localStorage
    } else {
        B.setAttribute("data-bs-theme", "light");
        localStorage.setItem("theme", "light"); // Simpan mode ke localStorage
    }
    window.dispatchEvent(new Event("themeChanged")); // Opsional: Untuk men-trigger event jika diperlukan
}

// Fungsi untuk mendapatkan mode dari localStorage
function getSavedTheme() {
    return localStorage.getItem("theme") || "light"; // Default ke "light" jika tidak ada data
}

// Inisialisasi tema saat halaman dimuat
document.addEventListener("DOMContentLoaded", function () {
    const savedTheme = getSavedTheme(); // Ambil mode yang disimpan
    setThemeMode(savedTheme); // Terapkan mode yang disimpan
});

// Event listener untuk tombol toggle mode
const z = document.querySelectorAll(".light-dark-mode");
if (z.length) {
    z[0].addEventListener("click", function (e) {
        const B = document.documentElement; // Root element (<html>)
        const currentTheme = B.getAttribute("data-bs-theme");
