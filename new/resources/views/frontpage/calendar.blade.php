    ...
    <script>
         document.addEventListener('DOMContentLoaded', function() {
             ...
             function getEvents() {
                 ...
                 let url = "{{ route('customer.calendar') }}";
                 let params = [];
             ...
             const calendar = new FullCalendar.Calendar(calendarEl, {
                 ...
                 eventClick: function(info) {
                     ...
                     const detailUrl = "{{ route('customer.detail', ['order' => ':id']) }}".replace(':id', eventId);
                     fetch(detailUrl, {
                         method: 'GET',
                         headers: {
                             'Accept': 'application/json',
                     ...
                     .then(data => {
                        ...
                        if (phone) {
                            // Sembunyikan 6 digit terakhir untuk tampilan, tapi gunakan nomor asli untuk link
                            const maskedPhone = phone.slice(0, -6) + 'xxxxxx';
                            document.getElementById('modal-phone').innerHTML = `
                                <a href="https://wa.me/${phone}" target="_blank">+${maskedPhone}</a>
                            `;
                        } else {
                            document.getElementById('modal-phone').innerHTML = '-';
                        }
                        ...
                        function maskMoney(value) {
                            ...
                            if (numeric === 0) return 'Rp. 0';

                            const str = numeric.toString();
                            return 'Rp. ' + (str.length > 2 ? str.slice(0, 2) + 'xxxxx' : 'xxxxx');
                        }

                        document.getElementById('modal-rental-price').textContent =
                            data.rental_price != null ? maskMoney(data.rental_price) : '-';

                        document.getElementById('modal-down-payment').textContent =
                            data.down_payment != null ? maskMoney(data.down_payment) : '-';

                        document.getElementById('modal-remaining-cost').textContent =
                            data.remaining_cost != null ? maskMoney(data.remaining_cost) : '-';
